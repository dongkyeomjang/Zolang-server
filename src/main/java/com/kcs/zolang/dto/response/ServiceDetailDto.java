...
public record ServiceDetailDto(
        ...
        public record Status(
                @Schema(description = "Network(service) 상태") Map<String, String> serviceStatus) {
        }

        public static ServiceDetailDto fromEntity(V1Service service) {
                MetaData metaData = new MetaData(
                        service.getMetadata().getCreationTimestamp().toLocalDateTime().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")),
                        getAge(Objects.requireNonNull(service.getMetadata().getCreationTimestamp()).toLocalDateTime()),
                ...
                Spec spec = new Spec(
                        service.getSpec().getPorts().stream().map(V1ServicePort::getPort).collect(Collectors.toList()),
