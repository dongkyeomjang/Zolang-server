...
public record ServiceListDto(
    ...
    public static ServiceListDto fromEntity(V1Service service) {
        return ServiceListDto.builder()
            ...
            .serviceExternalIP(service.getStatus().getLoadBalancer().getIngress() == null ? null
                : service.getStatus().getLoadBalancer().getIngress().stream()
                    .map(V1LoadBalancerIngress::getHostname).toList())
            .servicePort(service.getSpec().getPorts().stream().map(V1ServicePort::getPort).toList())
            .serviceAge(getAge(Objects.requireNonNull(service.getMetadata().getCreationTimestamp())
                .toLocalDateTime()))
            .build();
    }
}
